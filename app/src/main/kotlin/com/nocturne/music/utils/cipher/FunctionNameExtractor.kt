...
object FunctionNameExtractor {
    ...
    private val SIG_FUNCTION_PATTERNS = listOf(
        ...
        // Generic 2025/2026: &&(VAR=FUNC(...decodeURIComponent...
        Regex("""&&\s*\(\s*[a-zA-Z0-9$]+\s*=\s*([a-zA-Z0-9$]+)\s*\([^)]*decodeURIComponent"""),
        // Function definition pattern: FUNC=function(a){a=a.split("")
        Regex("""\b([a-zA-Z0-9$]+)\s*=\s*function\s*\([a-zA-Z0-9$,\s]*\)\s*\{\s*[a-zA-Z0-9$]+\s*=\s*[a-zA-Z0-9$]+\.split\(""\)"""),
        ...
        Regex("""\.set\([^,]+\s*,\s*(?:encodeURIComponent\s*\()?\s*([a-zA-Z0-9$]+)\s*\("""),
    )
