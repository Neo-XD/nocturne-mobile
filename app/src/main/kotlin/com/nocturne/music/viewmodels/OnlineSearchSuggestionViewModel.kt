        ...
        viewModelScope.launch {
            combine(
                ...
                context.dataStore.data
                    .map { prefs ->
                        val ids = prefs[com.nocturne.music.constants.SearchListenHistoryKey]?.split(",")?.filter { it.isNotEmpty() } ?: emptyList()
                        ids.take(9).mapNotNull { id ->
                            database.getSongByIdBlocking(id)?.let { song ->
                                EventWithSong(
                                    com.nocturne.music.db.entities.Event(songId = id, timestamp = java.time.LocalDateTime.now(), playTime = 0),
                                    song
                                )
                            }
                        }
